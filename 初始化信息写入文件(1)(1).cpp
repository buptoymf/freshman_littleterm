...
main()
{
	...
	ScenicSpotList[num3].ScenicSpot_INCOME=0.0;//景点总收入量
	num3++; 
	//2号景点
	...
	//写入订单信息 
	FILE *fp4;
	fp4=fopen("订单信息.txt","wb");
	int num4=0;
	//1号订单
	strcpy(info[num4].order_ID1,"youke001");//游客ID 
	strcpy(info[num4].order_NUMBER,"jd0001");//景点ID 
	info[num4].order_IDENTITY1=10;//学生身份
	info[num4].order_IDENTITY2=10;//军人身份
	info[num4].order_IDENTITY3=10;//普通身份
	strcpy(info[num4].order_AGE,"30岁左右");//年龄段 
	info[num4].order_PEOPLENUM=30;//人数
	info[num4].order_YEAR=2018;//订单年份 
	info[num4].order_MONTH=9;//订单月份 
	info[num4].order_DAY=18;//订单日 
	info[num4].order_HOUR=1;//订单小时
	info[num4].order_MINUTE=20;//订单分钟
	strcpy(info[num4].order_ID,"youke0012018918120");//订单编号（游客ID+订票时间）
	info[num4].order_PRICE=100.00;//订单总价格
	strcpy(info[num4].order_jidu,"淡季");//订单所属季度
	num4++; 
	//2号订单
	strcpy(info[num4].order_ID1,"youke003");//游客ID 
	strcpy(info[num4].order_NUMBER,"jd0004");//景点ID 
	info[num4].order_IDENTITY1=20;//学生身份
	info[num4].order_IDENTITY2=30;//军人身份
	info[num4].order_IDENTITY3=40;//普通身份
	strcpy(info[num4].order_AGE,"50岁左右");//年龄段 
	info[num4].order_PEOPLENUM=90;//人数
	info[num4].order_YEAR=2017;//订单年份 
	info[num4].order_MONTH=10;//订单月份 
	info[num4].order_DAY=1;//订单日 
	info[num4].order_HOUR=12;//订单小时
	info[num4].order_MINUTE=30;//订单分钟
	strcpy(info[num4].order_ID,"youke00320171011230");//订单编号（游客ID+订票时间）
	info[num4].order_PRICE=200.00;//订单总价格
	strcpy(info[num4].order_jidu,"旺季");//订单所属季度
	num4++; 
	fwrite(&info,sizeof(info_list),num4,fp4);
	fclose(fp4);
}	
