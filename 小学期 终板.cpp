...
int main()
{
	...
	while(panduan!=0)
	{
		...
		{
			case 1:
				Youkedenglu();//n=1时进入游客登陆界面 
				panduan=0;
				break;
			case 2:
				Guanliyuandenglu();//n=2时进入管理员登陆界面 
				panduan=0;
				break;
			case 3:
				panduan=0;
...
void Youkedenglu() 
{
	...
	tourist_login=fopen("游客用户名和密码备份.txt","wb");//从文件中读出各类信息，并准备存入信息 
	int cal1=1,cal2,cal3=1,cal4,cal5=0;
	
	char tourist_ID2[20]={"\0"},tourist_PASSWORD2[20]={"\0"};
	printf("请输入用户名：\n");
	scanf("%s",tourist_ID2);
	fread(touristList,sizeof(struct tourist),5,tourist_ID1);//读出文件中的游客信息 
	//验证用户的用户名输入是否正确 
	while(cal3!=0)
	{
	for(cal2=0;cal2<5;cal2++)
		{
			if(strcmp(tourist_ID2,touristList[cal2].tourist_ID)==0)//若输入的信息在文件中存在，则可跳出循环 
			{
				cal1=0;
				cal3=0;
				cal4=cal2;
				strcpy(touristList1[cal2].tourist_ID1,tourist_ID2);//将输入的用户名存入备份中 
			//	youkeid=cal2;
			}
		}
		   if(cal1!=0)
	...
	for(cal2=0;cal2<5;cal2++)
		{
			if(strcmp(tourist_PASSWORD2,touristList[cal4].tourist_PASSWORD)==0)//若输入的信息在文件中存在，则可跳出循环
			{
				cal1=0;
				cal3=0;
				strcpy(touristList1[cal2].tourist_PASSWORD1,tourist_PASSWORD2);//将输入的密码存入备份中 
			}
		}
		   if(cal1!=0)
		  {
	...
	fwrite(&touristList1,sizeof(struct touristList1),1,tourist_login);//将保存的备份信息存入文件中 
	fclose(tourist_ID1);
...
void Jingdianchaxun()
{
	...
	printf("\t*       4.根据是否有折扣查询            *\n");
	printf("\t*       5.根据价格排序所有景点          *\n");
	printf("\t*       6.根据预定量排序所有景点        *\n");
	printf("\t*       7.根据好评率排序所有景点        *\n");
	printf("\t*****************************************\n");
	scanf("%d",&choose);
	while(panduan!=0)
	{
...
void Jingdianmingcheng()
{
	...
	FILE *Jingdian1;
	Jingdian1=fopen("景点信息.txt","rb");
	char jingdianchaxun1[80]={"\0"};
	printf("请输入要查询的景点名称：\n");
	scanf("%s",jingdianchaxun1);
	for(cal4=0;fread(&ScenicSpotList[cal4],sizeof(struct  ScenicSpot),1,Jingdian1)!=0;cal4++);//读取景点中的所有信息 
	while(cal3!=0) 
	{
		for(cal2=0;cal2<cal4;cal2++)
		{
		if(strcmp(jingdianchaxun1,ScenicSpotList[cal2].ScenicSpot_NAME)==0)//若文件中有输入的景点名称，则读取相应的景点信息 
			{
			printf("景点ID: "); 
		...
		int choose,panduan=1;
		scanf("%d",&choose);
	while(panduan!=0)//判断其输入数据正确与否，正确则进行相应的程序，否则要求重新输入 
	{
		switch(choose)
		{
			case 1:
...
void Jingdianquyu()
{
	int cal1=1,cal2,cal3=1,cal4;
	FILE *Jingdian3;
	Jingdian3=fopen("景点信息.txt","rb");
	char jingdianchaxun3[80]={"\0"};
	printf("请输入要查询的景点区域：\n");
	scanf("%s",jingdianchaxun3);
	for(cal4=0;fread(&ScenicSpotList[cal4],sizeof(struct  ScenicSpot),1,Jingdian3)!=0;cal4++);//读取景点中的所有信息 
	while(cal3!=0) 
	{
		for(cal2=0;cal2<cal4;cal2++)//（暂时定为八个） 
		{
		if(strcmp(jingdianchaxun3,ScenicSpotList[cal2].ScenicSpot_AREA)==0)//若文件中有输入的景点区域，则读取相应的景点信息 
			{
			printf("景点ID: "); 
		    puts(ScenicSpotList[cal2].ScenicSpot_ID);
	        printf("景点名称: ");
...
void Shifouzhekou()
{
	...
	FILE *Jingdian4;
	Jingdian4=fopen("景点信息.txt","rb");
	for(cal4=0;fread(&ScenicSpotList[cal4],sizeof(struct  ScenicSpot),1,Jingdian4)!=0;cal4++);//读取景点中的所有信息 
	printf("\t**************Tourist Menu***************\n");
	printf("\t*     请问您要查询哪类折扣信息：        *\n");
	printf("\t*     1.学生                            *\n");
	printf("\t*     2.军人                            *\n");
	...
	while(cal7!=0) 
	{
		switch(choose)
		{
			...
			{
				if(ScenicSpotList[cal1].ScenicSpot_DISCOUNT1 < 1)//若有折扣，则输出相应的信息 
				{
			printf("景点ID: "); 
			...
			{
			if(ScenicSpotList[cal3].ScenicSpot_DISCOUNT2 < 1)//若有折扣，则输出相应的信息 
			{
			printf("景点ID: "); 
			...
			{
			if(ScenicSpotList[cal5].ScenicSpot_DISCOUNT3 < 1)//若有折扣，则输出相应的信息 
			{
			printf("景点ID: "); 
...
void piaojiapaixu()
{
   ...
   fp=fopen("景点信息.txt","rb");//打开文件“景点信息” 
	...
	while(panduan!=0)
	{
		...
		scanf("%d",&choose1);
	while(panduan1!=0)//判断其输入数据正确与否，正确则进行相应的程序，否则要求重新输入 
	{
		switch(choose1)
		{
			case 1:
...
/*	int cal2;
	...
	FILE *tourist_ID1=fopen("游客信息.txt","rb");
	FILE *fp=
	...
	*/ 
	...
	int a;//判断订了第几个景点的票 
	int b,c,d,f,g,h,j,k,l;//判断各项信息输入是否正确的标记值 
...
//	char buf2[1000];
	...
	jingdian=fopen("景点信息.txt","rb");//打开景点信息文件以判断景点id正误以及读取景点票数 
...
//	printf("景点数量：%d\n",jingdianshuliang);
		...
		for(y=0;y<jingdianshuliang;y++)//将输入内容与各景点id逐一比较 
		{
...
//		}
		...
		//根据订单日期生成订单编号 
		strcpy(info[i].order_ID1,tourist_ID2);//游客ID待读取 
...
/*	while(fgets(buf,1000,pf2))
	...
	//判断时间是否合适（判别方法为分别算出两个时间距离公元0年1月1日0点的时间差，然后相减） 
...
FILE  *fp,*fp2;
        ...
        fgets(tourist_ID2,sizeof(tourist_ID2),fp2);
        printf("※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※\n");
        printf("                                查询结果\n");
        printf("------------------------------------------------------------------------\n");
        printf("游客团体ID\t游客团体密码\t游客团体联系人电话\t游客团邮箱\n");
		printf("------------------------------------------------------------------------\n");
		for(int i=0;fread(&touristList[i],sizeof(struct  tourist),1,fp)==1;i++)
        	if(strcmp(touristList[i].tourist_ID,tourist_ID2)==0)
        	{
				printf("%s\t%s\t",touristList[i].tourist_ID,touristList[i].tourist_PASSWORD);
				printf("%s\t\t%s\n",touristList[i].tourist_PHONE,touristList[i].tourist_MAIL);
			}
		printf("------------------------------------------------------------------------\n\n");
        printf("※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※\n\n\n");
        fclose(fp);
 ...
 {  
    ...
    scanf("%d",&panduan);//选择修改密码或邮箱 
     ...
     fp2=fopen("游客用户名和密码备份.txt","rb");
       ...
       fread(&touristList1[0],sizeof(struct tourist),1,fp2);//读取备份中内容
    int i,n;       
    for(n=0;n<3;n++)
        {
  ...
  {      
	 ...
	 {	  
       ...
       if(strcmp(mima,touristList1[i].tourist_PASSWORD1)==0)
       {
       	 printf("请输入新密码\n"); 
		 ...
		 scanf("%s",&mail);
		   int a,d;
			...
			FILE *fp3;
			 ...
			 strcpy(touristList[i].tourist_MAIL,mail);
...
void Guanliyuandenglu()
{
	...
	administrator_login=fopen("管理员用户名和密码备份.txt","wb");//从文件中读出各类信息，并准备存入信息 
	int cal1=1,cal2,cal3=1,cal4,cal5=0;
	char administrator_ID2[20]={"\0"},administrator_PASSWORD2[20]={"\0"};
	printf("请输入管理员ID：\n");
	scanf("%s",administrator_ID2);
	fread(administratorList,sizeof(struct administrator),3,administrator_ID1);//读出文件中的管理员信息 
	//验证用户的用户名输入是否正确 
	while(cal3!=0)
	{
	for(cal2=0;cal2<3;cal2++)
		{
			if(strcmp(administrator_ID2,administratorList[cal2].administrator_ID)==0)//若输入的信息在文件中存在，则可跳出循环 
			{
				cal1=0;
				cal3=0;
				cal4=cal2;
				strcpy(administratorList1[cal2].administrator1_ID,administrator_ID2);//将输入的用户名存入备份中 
			}
		}
		   if(cal1!=0)
		  {
	...
	for(cal2=0;cal2<3;cal2++)
		{
			if(strcmp(administrator_PASSWORD2,administratorList[cal4].administrator_PASSWORD)==0)//若输入的信息在文件中存在，则可跳出循环
			{
				cal1=0;
				cal3=0;
				strcpy(administratorList1[cal2].administrator1_PASSWORD,administrator_PASSWORD2);//将输入的密码存入备份中 
			}
		}
		   if(cal1!=0)
		  {
	...
	fwrite(administratorList1,sizeof(administratorList1),1,administrator_login);//将保存的备份信息存入文件中 
...
void dingdantongji()
{
	...
	printf("\t*      2.按访问人数对景点进行排序         *\n");
	printf("\t*      3.按各景点的利润排序               *\n");
	printf("\t*      4.统计不同景点的用户年龄层次分布   *\n");
	printf("\t*      5.按季度对不同景点的售票情况排序   *\n");
	printf("\t*      6.退出程序                         *\n");
	printf("\t*******************************************\n");
	scanf("%d",&choose);
	while(panduan!=0)
...
{ 
	...
	//d为订单个数 
	  ...
	  {	  
	    for(int n=0;n<d;d++)
	      {
	    	if(strcmp(ScenicSpotList[k].ScenicSpot_ID,info[n].order_NUMBER)==0)
	    	 ScenicSpotList[k].ScenicSpot_INCOME=-ScenicSpotList[k].ScenicSpot_COST+info[n].order_PRICE;
	      }
	  }    //根据景点ID分别计算各景点的利润 
	  if(d==0)
	    for(int k=0;k<i;k++)
	   ScenicSpotList[k].ScenicSpot_INCOME=-ScenicSpotList[k].ScenicSpot_COST;
	int j,n;
	float k;//n,j,k用来辅助排序， ScenicSpotList2用来辅助排序 
	 for(int n=1;n<i;n++)
   {
      k=ScenicSpotList[n].ScenicSpot_INCOME;
      ScenicSpotList2[n-1]=ScenicSpotList[n];//存下 ScenicSpotList[n]数据 
      j=n-1;//j用来计数当前数据最多与前数据交换次数 
        while((j>=0)&&(ScenicSpotList[n].ScenicSpot_INCOME<k))
        // j不为负且ScenicSpotList[n-1]>ScenicSpotList[n],则交换
      {
         ScenicSpotList[j+1].ScenicSpot_INCOME=ScenicSpotList[j].ScenicSpot_INCOME;
         ScenicSpotList2[j+1]=ScenicSpotList[j];         
         j--;
      } //j-1，继续比较 ScenicSpotList[n-2]>ScenicSpotList[n]
     ScenicSpotList[j+1].ScenicSpot_INCOME=k;
     ScenicSpotList[j+1]=ScenicSpotList2[n-1];
      }
      printf("   景点名称          景点ID          利润\n");
    for(int n=0;n<i-1;n++)
    {       
       printf("%11s\t%11s\t%11.2f\n",ScenicSpotList[n].ScenicSpot_NAME,ScenicSpotList[n].ScenicSpot_ID,ScenicSpotList[n].ScenicSpot_INCOME);
    }//输出排序后景点数据 
      fclose(fp);//关闭文件 	
   ...
   fp=fopen("景点信息.txt","rb");//打开文件“景点信息” 
    ...
    int yu[80];
     ...
     for(int n=0;n<i;n++)
      { 
	    for(int j=0;j<i2;j++)
         if(strcmp(ScenicSpotList[n].ScenicSpot_ID,info[j].order_NUMBER)==0&&info[j].order_MONTH>0&&info[j].order_MONTH<4)
           yu[n]=yu[n]+info[j].order_IDENTITY1+info[j].order_IDENTITY2+info[j].order_IDENTITY3;
	  }
	  
    //根据景点id，分别计算不同季度各景点预定量 
    for(int n=0;n<i;n++)
     {
        
     	ScenicSpotList[n].ScenicSpot_YUDING=yu[n];
	 }
	 // 输入修改后预定量 
     fclose(fp);
     fclose(fp2);
	 ...
	 for(int n=1;n<i;n++)
    {
     ...
     for(int n=0;n<i;n++)
      { 
	    for(int j=0;j<i2;j++)
         if(strcmp(ScenicSpotList[n].ScenicSpot_ID,info[j].order_NUMBER)==0&&info[j].order_MONTH>3&&info[j].order_MONTH<7)
           yu[n]=yu[n]+info[j].order_IDENTITY1+info[j].order_IDENTITY2+info[j].order_IDENTITY3;
	  } 
    //根据景点id，分别计算各景点预定量 
    for(int n=0;n<i;n++)
	 ...
	 for(int n=1;n<i;n++)
    {
     ...
     ScenicSpotList[j+1]=ScenicSpotList2[n-1];
      ...
      { 
	    for(int j=0;j<i2;j++)
         if(strcmp(ScenicSpotList[n].ScenicSpot_ID,info[j].order_NUMBER)==0&&info[j].order_MONTH>6&&info[j].order_MONTH<10)
           yu[n]=yu[n]+info[j].order_IDENTITY1+info[j].order_IDENTITY2+info[j].order_IDENTITY3;
	  } 
    //根据景点id，分别计算各景点预定量 
    for(int n=0;n<i;n++)
	 ...
	 for(int n=1;n<i;n++)
    {
     ...
     ScenicSpotList[j+1]=ScenicSpotList2[n-1];
      ...
      { 
	    for(int j=0;j<i2;j++)
         if(strcmp(ScenicSpotList[n].ScenicSpot_ID,info[j].order_NUMBER)==0&&info[j].order_MONTH>9&&info[j].order_MONTH<13)
           yu[n]=yu[n]+info[j].order_IDENTITY1+info[j].order_IDENTITY2+info[j].order_IDENTITY3;
	  } 
    //根据景点id，分别计算各景点预定量 
    for(int n=0;n<i;n++)
