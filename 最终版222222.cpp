...
void inputinfo()
{
	...
	int panduan,panduan2;//判断各项信息输入是否正确的标记值 
	...
	fread(&ScenicSpotList,sizeof(struct  ScenicSpot),1,fp);
	...
	for(i=0;i<n;i++)
	{
		...
		fseek(jingdian,0L,0);
		for(i=0;fread(&ScenicSpotList[i],sizeof(struct  ScenicSpot),1,jingdian)!=0;i++)
		{
			fread(&ScenicSpotList,sizeof(struct  ScenicSpot),1,jingdian);
		}
...
//		}
	...
	//		}
			...
			for(i=0;fread(&ScenicSpotList[i],sizeof(struct  ScenicSpot),1,jingdian)!=0;i++)
		{
			fread(&ScenicSpotList[a].ScenicSpot_PRICE1,sizeof(struct  ScenicSpot),1,jingdian);
		}
		...
		
		{
			...
			fread(&ScenicSpotList[a].ScenicSpot_PRICE2,sizeof(struct  ScenicSpot),1,jingdian);
...
//		}
			...
			fread(&ScenicSpotList[a],sizeof(struct  ScenicSpot),1,jingdian);
...
//	{
	for(i=0;fread(&ScenicSpotList[i],sizeof(struct  ScenicSpot),1,jingdian)!=0;i++)
	{ 
	fread(&ScenicSpotList[i],sizeof(struct  ScenicSpot),1,jingdian);
		
	
		
//		if(i%13==1) 
		printf("景点ID：%s",ScenicSpotList[i].);
