...
void Youkedenglu()
{
	...
	tourist_login=fopen("游客用户名和密码备份.txt","w");
	...
	for(cal2=0;cal2<5;cal2++)
		{
		   ...
		   if(cal1!=0)
		  {
			...
			if(cal5 == 2)
			{
				printf("密码输入错误次数超过3次，请重新进入程序登陆\n");
				exit(0);
				cal1=0;
				cal3=0;
			}
	} 
	fwrite(touristList1,sizeof(touristList1),1,tourist_login);
	fclose(tourist_ID1);
...
void youketuanxiugai()
{
    ...
    scanf("%d",&panduan);//选择修改密码或邮箱 
     ...
     FILE *fp,*fp2;
     fp=fopen("游客团信息.txt","rb");
       fread(&touristList,sizeof(struct tourist),5,fp);//读取游客团信息中内容
     fp2=fopen("游客团用户名和密码备份.txt","rb");
      
       fread(&touristList1,sizeof(struct tourist),1,fp2);//读取备份中内容
    int i,n;       
    for(n=0;n<3;n++)
        {
        	if(strcmp(touristList[n].tourist_ID,touristList1[n].tourist_ID1)==0)
			   i=n;
	    }//判断所修改的信息是第几个游客团的，i为该管理员序号 
    printf("%s\n",touristList1[n].tourist_PASSWORD1);
   switch (panduan)       
  {      
      case 1:  //1为对密码的修改
  	{	 
	 ...
	 {	  
       ...
       if(strcmp(mima,touristList1[n].tourist_PASSWORD1)==0)
       {
       	 printf("请输入新密码\n"); 
...
void Guanliyuandenglu()
{
	...
	administrator_login=fopen("管理员用户名和密码备份.txt","w");
	int cal1=1,cal2,cal3=1,cal4,cal5=0;
...
void age() 
 {
   ...
   void guanliyuanxiugai()//修改管理员密码或邮箱 
 {  
    void guanliyuancaidan();
    printf("1.密码修改\n2.邮箱修改\n3.取消修改\n");
    int panduan;
  ...
  {      
     ...
     //if (strcmp(mima,administratorList2[0].administrator_PASSWORD)==0)
       if(strcmp(mima,administratorList1[n].administrator1_PASSWORD)==0)
       {
       	 printf("请输入新密码\n"); 
